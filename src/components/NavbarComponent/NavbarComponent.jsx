...
const NavbarComponent = (props) => {
    ...
    const {current, categoryNameChild, setPanigate,categoryNameParent, setCategoryNameChild, setCategoryNameParent, setNameCategorySelected, setNameCategoryChildSelected } = props
        ...
        return itemsChildAndParent.flatMap((item, index) => {
            if (item?._id?.parentId == id) {
                return item?.names?.map((name, index) => {
                    setCategoryNameChild(prev => [...prev, { id: item?.idChild[index],parentId:item?._id?.parentId, name: name }])
